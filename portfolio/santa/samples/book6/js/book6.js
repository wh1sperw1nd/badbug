...
var sampleName = 'book6',
	samplePath = 'samples/book6/',
	zoomIn = false;
...
function updateDepth(book, newPage) {
	...
	if (newPage>3)
		...
		$('.book6 .p2 .depth').css({width: 0});

		depthWidth = Math.round(16*Math.min(1, (pages-page)*2/pages));

	if (newPage<pages-3)
		$('.book6 .p111 .depth').css({
			width: depthWidth,
			right: 20 - depthWidth
		});
	else
		$('.book6 .p111 .depth').css({width: 0});
...
function loadFlipbook(flipbook) {
	...
	flipbook.turn({
		...
		turning: function(e, page, view) {
			...
			if (page>=2)
				$('.book6 .p2').addClass('fixed');
			else
				$('.book6 .p2').removeClass('fixed');

			if (page<book.turn('pages'))
				$('.book6 .p111').addClass('fixed');
			else
				$('.book6 .p111').removeClass('fixed');
