...
PlayLion.prototype.init = function () {
...
PlayLion.prototype.initScreenAnd3D = function() {
    ...
    this.container = document.getElementById('lionWorld');
    this.container.appendChild(this.renderer.domElement);
    this.renderer.domElement.setAttribute("id","lionWorldCanvas");
...
PlayLion.prototype.onWindowResize = function() {
    this.HEIGHT = window.innerHeight;
    this.WIDTH = window.innerWidth;
    this.windowHalfX = this.WIDTH / 2;
    this.windowHalfY = this.HEIGHT / 2;
...
PlayLion.prototype.handleMouseMove = function(event) {
    this.mousePos = {x: event.clientX, y: event.clientY};
}

PlayLion.prototype.handleMouseDown = function(event) {
    this.isBlowing = true;
}
PlayLion.prototype.handleMouseUp = function(event) {
    this.isBlowing = false;
}

PlayLion.prototype.handleTouchStart = function(event) {
...
PlayLion.prototype.handleTouchEnd = function(event) {
    this.mousePos = {x: this.windowHalfX, y: this.windowHalfY};
    this.isBlowing = false;
}

PlayLion.prototype.handleTouchMove = function(event) {
    if (event.touches.length == 1) {
        event.preventDefault();
        this.mousePos = {x: event.touches[0].pageX, y: event.touches[0].pageY};
    }
...
PlayLion.prototype.createLights = function() {
...
PlayLion.prototype.createFloor = function() {
    this.floor = new THREE.Mesh(new THREE.PlaneBufferGeometry(1000, 500), new THREE.MeshBasicMaterial({color: 0xebe5e7}));
...
PlayLion.prototype.createLion = function() {
    this.lion = new Lion();
    this.scene.add(this.lion.threegroup);
}

PlayLion.prototype.createFan = function() {
    this.fan = new Fan();
...
Fan.prototype.update = function (xTarget, yTarget) {
    ...
    this.tPosX = currentPage.rule3(xTarget, -200, 200, -250, 250);
    this.tPosY = currentPage.rule3(yTarget, -200, 200, 250, -250);
...
Lion.prototype.look = function (xTarget, yTarget) {
    this.tHeagRotY = currentPage.rule3(xTarget, -200, 200, -Math.PI / 4, Math.PI / 4);
    this.tHeadRotX = currentPage.rule3(yTarget, -200, 200, -Math.PI / 4, Math.PI / 4);
    this.tHeadPosX = currentPage.rule3(xTarget, -200, 200, 70, -70);
    this.tHeadPosY = currentPage.rule3(yTarget, -140, 260, 20, 100);
    this.tHeadPosZ = 0;
    ...
    this.tIrisYScale = 1;
    this.tIrisZScale = 1;
    this.tIrisPosY = currentPage.rule3(yTarget, -200, 200, 35, 15);
    this.tLeftIrisPosZ = currentPage.rule3(xTarget, -200, 200, 130, 110);
    this.tRightIrisPosZ = currentPage.rule3(xTarget, -200, 200, 110, 130);
    ...
    this.tRightKneeRotZ = currentPage.rule3(xTarget, -200, 200, .3 - Math.PI / 8, .3 + Math.PI / 8);
    this.tLeftKneeRotZ = currentPage.rule3(xTarget, -200, 200, -.3 - Math.PI / 8, -.3 + Math.PI / 8)
...
Lion.prototype.cool = function (xTarget, yTarget) {
    ...
    this.tHeadRotX = currentPage.rule3(yTarget, -200, 200, Math.PI / 4, -Math.PI / 4);
    this.tHeadPosX = currentPage.rule3(xTarget, -200, 200, -70, 70);
    this.tHeadPosY = currentPage.rule3(yTarget, -140, 260, 100, 20);
    this.tHeadPosZ = 100;
    ...
    this.tLipsPosX = currentPage.rule3(xTarget, -200, 200, -15, 15);
    this.tLipsPosY = currentPage.rule3(yTarget, -200, 200, -45, -40);

    this.tMouthPosZ = 168;
    this.tSmilePosX = currentPage.rule3(xTarget, -200, 200, -15, 15);
    this.tSmilePosY = currentPage.rule3(yTarget, -200, 200, -20, -8);
    this.tSmilePosZ = 176;
    this.tSmileRotZ = currentPage.rule3(xTarget, -200, 200, -Math.PI - .3, -Math.PI + .3);
...
PlayLion.prototype.loop = function() {
    this.render();
...
//    var requestAnimLion = requestAnimationFrame(function(){
//        if(currentPage){
//            currentPage.loop();
//            console.log("lion");
//        }
...
PlayLion.prototype.render = function() {
...
PlayLion.prototype.clamp = function(v, min, max) {
    return Math.min(Math.max(v, min), max);
}

PlayLion.prototype.rule3 = function(v, vmin, vmax, tmin, tmax) {
