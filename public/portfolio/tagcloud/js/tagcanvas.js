...
Outline.prototype.Update = function(x,y,w,h,sc,p) {
...
Outline.prototype.Draw = function(c) {
	...
	if(t.pulsateTo < 1.0)
		c.globalAlpha = t.pulsateTo + ((1.0 - t.pulsateTo) * 
			(0.5 + (cos(2 * Math.PI * diff / (1000 * t.pulsateTime)) / 2.0)));
	else
		c.globalAlpha = 1.0;
...
function Tag(tc,name,a,v,w,h) {
	...
	if(this.image) {
		if(tc.txtOpt && (tc.shadowBlur || tc.shadowOffset[0] || tc.shadowOffset[1])) {
...
Tag.prototype.DrawImage = function(c,xoff,yoff) {
	var t = this.tc, x = this.x, y = this.y, w, h, s = this.sc, o = this.outline, i = this.image;
	c.globalAlpha = this.alpha;
	c.setTransform(s,0,0,s,0,0);
	c.fillStyle = this.colour;
